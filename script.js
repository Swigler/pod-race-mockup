 ...
 */
 ...
 // You can change this to match your backend API endpoint
 let SERVER_URL = "http://localhost:9000";
 
 // Function to update the SERVER_URL (can be called from the console)
 function setServerUrl(url) {
     SERVER_URL = url;
     console.log(`Backend URL updated to: ${SERVER_URL}`);
     return `Backend URL updated to: ${SERVER_URL}`;
 }
 let racers = {};
 ...
 function appendDebug(message) {
  ...
  async function check_queue_status(userId) {
      ...
      try {
          ...
          if (!response.ok) {
              appendDebug(`Queue status check error: HTTP ${response.status}`);
              return null;
          }
          ...
          appendDebug(`Queue status check error: ${error.message}`);
          return null;
      }
  }
  ...
  // Function to update the backend URL from the UI
  function updateBackendUrl() {
      ...
      if (newUrl) {
          ...
          appendDebug("Please enter a valid URL");
      }
  }
  ...
  document.addEventListener("DOMContentLoaded", function() {
      ...
      document.getElementById("updateUrlButton").addEventListener("click", updateBackendUrl);
      updateButtonState();
      updateUserCards();
      ...
      document.getElementById("backendUrl").addEventListener("keypress", function(e) {
          if (e.key === "Enter") updateBackendUrl();
      });
  });
